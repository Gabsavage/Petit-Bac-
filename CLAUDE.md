...
**Sortir du salon** se fait par le bouton de gauche de la barre du haut. `#back-btn` a donc deux rôles selon l'écran, et `renderTop()` change son `data-action` (`go-home` ↔ `leave-room`) en plus de son icône et de son `aria-label` — changer l'icône seule laisserait le mauvais handler branché. Il n'apparaît **qu'au salon** : en pleine manche, une sortie à l'endroit où le pouce cherche « retour » ferait partir des gens par accident. C'est aussi pourquoi `handleLeaveRoom` demande confirmation — partir efface le PIN de l'URL (`goHome` fait un `replaceState`), et revenir suppose le QR, le lien ou la liste des parties en cours. Le lien « Quitter la partie » de la feuille de réglages reste en place, c'est le même handler.
