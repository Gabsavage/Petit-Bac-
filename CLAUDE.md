...
- **Profil et amis sont 100% locaux** (`pb_profile`, `pb_friends`), rien ne part dans Firebase — c'est un choix, pas un oubli : publier un annuaire de joueurs dans une base aux règles ouvertes posait un problème de vie privée pour un gain faible. Les amis sont donc une mémoire des gens croisés en partie (remplie par `recordFriends()` en même temps que les stats), sans présence ni invitation possible. Si un jour on veut "voir quels amis jouent et les rejoindre", il faudra un identifiant de joueur stable et un nœud `users/` — aujourd'hui la clé d'un joueur est dérivée de son prénom (`keyFor`), donc deux "Marc" se confondent.
